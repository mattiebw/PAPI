...
{
    glm::ivec2 tileCoords = glm::ivec2(std::floor(static_cast<float>(x) / m_ChunkSize.x), std::floor(static_cast<float>(y) / m_ChunkSize.y));
    auto chunk = m_Chunks.find(tileCoords);
    if (chunk == m_Chunks.end())
    {
        // MW @todo: Create the chunk.
...
bool TileMap::RectOverlapsSolidTile(const Rect &rect)
{
    ...
    {
        for (int x = static_cast<int>(std::floor(rect.Position.x)); x < static_cast<int>(std::ceil(rect.Position.x + rect.Size.x)); x++)
        {
